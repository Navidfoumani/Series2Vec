...
def WISDM(window_size, step):
    # Build data
    Data = {}
    # Get the current directory path
    current_path = os.getcwd()
    data_path = os.path.join(current_path, 'Datasets/WISDM/WISDM.npy')
    if os.path.exists(data_path):
        logger.info("Loading preprocessed WISDM data ...")
        ...
        if not os.path.exists(current_path + '/Datasets/WISDM/'):
            os.makedirs(current_path + '/Datasets/WISDM/')
        np.save(current_path + '/Datasets/WISDM/WISDM.npy', Data, allow_pickle=True)
...
def Downloader(current_path):
    ...
    path_to_check = os.path.join(current_path, 'WISDM/WISDM_ar_v1.1')
    ...
    if not os.path.exists(path_to_check):
        ...
        if response.status_code == 200:
            ...
            file_path = os.path.join(path_to_check, 'WISDM_ar_latest.tar.gz')
            ...
            print('WISDM.tar.gz Datasets downloaded and extracted successfully.')
        else:
            print('Failed to download the WISDM.tar.gz please update the file_url')
    else:
        print('WISDM Datasets Raw file already exists')
    return


def generate_data(current_path):
    directory = current_path + '/WISDM/WISDM_ar_v1.1/WISDM_ar_v1.1/WISDM_ar_v1.1_raw.txt'
    file = open(directory)
...
def load_activity(df):
    ...
    # Subject 27 and 36 are the only individuals who have both Sitting and Standing records. To split the data,
    # we assign one of them to the train set and the other to the test set.
    all_series = np.delete(all_series, np.where(all_series == '36'))
    all_series = np.delete(all_series, np.where(all_series == '27'))
    train_series, test_series = train_test_split([x for x in range(len(all_series))], test_size=2, random_state=1)
    train_series = all_series[train_series]
    test_series = all_series[test_series]
    train_series = np.append(train_series, '27')
    test_series = np.append(test_series, '36')
